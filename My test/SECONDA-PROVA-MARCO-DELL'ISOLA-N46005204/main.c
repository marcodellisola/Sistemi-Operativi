...
int main(){
        ...
        int ds_shm = shmget(shm_key,sizeof(message),IPC_CREAT | 0664);
	...
	pc->totale_elementi = 0;
	pc->coda = 0;
	pc->testa = 0;
	init_monitor(&(pc->m), total_process);
	...
	msgctl(key_queue_gen_filter,IPC_RMID,0);
	msgctl(key_queue_filter_checksum,IPC_RMID,0);
	msgctl(key_queue_checksum_visual,IPC_RMID,0);
	remove_monitor(&(pc->m));
