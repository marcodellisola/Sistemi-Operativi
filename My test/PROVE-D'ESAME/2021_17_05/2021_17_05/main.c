...
int main(int argc, char *argv[])
{
	...
	MonitorPC * p = malloc(sizeof(MonitorPC));
    	inizializza(p);

	for(i=0; i<2; i++) {
		/* TBD: avviare 2 produttori di tipo 1 */
		pthread_create(&thread_produttori_1[i],&attr,produttore_1,(void*)p);
	}

	for(i=0; i<2; i++) {
		/* TBD: avviare 2 produttori di tipo 2 */
		pthread_create(&thread_produttori_2[i],&attr,produttore_2,(void*)p);
	}

	/* TBD: avviare consumatore di tipo 1 */
	pthread_create(&thread_consumatore_1,&attr,consumatore_1,(void*)p);

	/* TBD: avviare consumatore di tipo 2 */
	pthread_create(&thread_consumatore_1,&attr,consumatore_2,(void*)p);
